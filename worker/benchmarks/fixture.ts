...
const sqlLiteral = (value: FixtureValue): string => {
...
export const fixtureToSql = (fixture: RepresentativeFixture): string => {
    const statements = [
        `-- Larafeed representative D1 fixture (${fixture.config.profile})`,
        `-- users=${fixture.config.users} feeds=${fixture.config.feeds} entries_per_feed=${fixture.config.entriesPerFeed}`,
        'PRAGMA foreign_keys = ON;',
    ];
    for (const [table, data] of Object.entries(fixture.tables)) {
        for (const row of data.rows) {
            statements.push(
                `INSERT INTO ${table} (${data.columns.join(', ')}) VALUES (${row.map(sqlLiteral).join(', ')});`,
            );
        }
    }
...
const jsonValue = (value: FixtureValue): unknown =>
    value instanceof Uint8Array ? { $blob: hex(value) } : value;

export const fixtureToNdjson = (fixture: RepresentativeFixture): string => {
    const lines = [
        JSON.stringify({
            type: 'fixture',
